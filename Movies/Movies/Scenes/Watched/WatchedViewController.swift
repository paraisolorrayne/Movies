...
protocol WatchedDisplayLogic: class {
    func displayFetchFromDataStoreResult(with viewModel: WatchedModels.FetchFromDataStore.ViewModel)  
    func displayWatchedResult(with viewModel: WatchedModels.Watched.ViewModel)
}

class WatchedViewController: UIViewController, WatchedDisplayLogic {

    // MARK: Properties

    var router: (NSObjectProtocol & WatchedRoutingLogic & WatchedDataPassing)?
    var interactor: WatchedBusinessLogic?
    ...
    private func setup() {
        ...
        let interactor = WatchedInteractor()
        let presenter = WatchedPresenter()
        let router = WatchedRouter()
    ...
    func setupFetchFromDataStore() {
        let request = WatchedModels.FetchFromDataStore.Request()
    ...
    @IBAction func someButtonClicked(_ sender: Any) {
        let request = WatchedModels.Watched.Request(variableToPass: someLabel.text)
        interactor?.Watched(with: request)
    }
